...
public class AvatarService {
	...
	public Bitmap get(MucOptions mucOptions, int size) {
		...
		if (count == 0) {
			...
			String letter =  getLetter(name); //name.substring(0, 1);
	...
	public Bitmap get(final String name, final int size) {
		...
		int color;
		if (name.length() > 0) {
			letter =  getLetter(name); //name.substring(0, 1);
    ...
    private String getLetter(String name) {
        if(Character.isDigit(name.charAt(0)) && name.contains("@")) name = name.substring(0, name.indexOf("@"));
        if(name.matches("-?\\d+(\\.\\d+)?")) {
